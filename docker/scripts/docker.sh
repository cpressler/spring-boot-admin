...
if [ "$2" == "i" ]; then
  ...
  docker build -f docker/server/spring-boot-admin.Dockerfile . --tag softvisionlvcp/spring-boot-admin:$tagversion --build-arg buildversion=`echo $BUILDVERSION`
else
...
if [ "$2" == "p" ]; then
  ...
  # Push Image to repository
  docker push softvisionlvcp/spring-boot-admin:$tagversion
else
